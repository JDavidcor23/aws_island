...
- [ ] **12. Probar los casos que rompen**
  - [ ] **`R` después de la escena** → vuelve a arrancar desde el principio, héroe fuera de pantalla. _(Si usaste variable de módulo en la tarea 7, acá revienta.)_
  - [ ] **`R` durante la escena** → reinicia sin quedar trabado.
  - [ ] **Pies en el piso** durante toda la caminata, sin flotar ni hundirse.
  - [ ] **Sin fondo:** renombrá `scene_island_path.png` → sigue funcionando con color plano.
  - [ ] **Sin un frame de caminata:** renombrá `hero_walk_3.png` → sigue funcionando.
  - [ ] **HUD apagado** en toda la escena, incluida la barra del jefe de Jennifer.
...
- [ ] **14. Repaso final antes de pedir merge**
  - [ ] Cero valores mágicos: todo sale de `INTRO_SCENE.js` o de constantes que ya existían.
  - [ ] Todo el movimiento usa `dt`. Nada de sumar píxeles fijos por frame.
  - [ ] `Math.round()` en todas las coordenadas de `drawImage`.
  - [ ] Cero `console.log`. Cero código muerto (`drawIntroScreen` borrado si quedó sin uso).
  - [ ] No agregaste fases a `GAME_STATES` ni campos a `createInitialState()`.
  - [ ] No modificaste las reglas de combate más allá del caso `INTRO` de `advance()`.
  - [ ] `git status`: tus 3 archivos nuevos + `GameEngine.js`, `battleLogic.js`, `drawScreens.js`, `ASSETS_MANIFEST.js`. Nada más.
  - [ ] El juego se juega completo: escena → 4 rondas → remate → victoria.
